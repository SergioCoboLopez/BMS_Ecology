...
class Parallel():
    ...
    def mcmc_step(self, verbose=False, p_rr=0.05, p_long=.45):
        ...
        for T, tree in list(self.trees.items()):
            # Sequential execution of the MCMC step
            #tree.mcmc_step(verbose=verbose, p_rr=p_rr, p_long=p_long)
            #"""   
        # Multi-Thread (threading lib) execution of the MCMC step. Each temperature in a different thread.
            x = threading.Thread(target=tree.mcmc_step, kwargs={'verbose':verbose, 'p_rr':p_rr, 'p_long':p_long})
            threads.append(x)
            x.start()
        for t in threads:
            t.join()
        #"""
        """
        # Multi-Process (multiprocessing lib) execution of the MCMC step. Each temperature in a different thread.
            x = multiprocessing.Process(target=tree.mcmc_step, kwargs={'verbose':verbose, 'p_rr':p_rr, 'p_long':p_long})
            threads.append(x)
            x.start()
        for t in threads:
            t.join()
